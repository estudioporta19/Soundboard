document.addEventListener('DOMContentLoaded', () => {
    ...
    // Se o seu HTML NÃO TIVER estes elementos, estas referências serão 'null'
    // e as funções do modal não serão executadas, o que está OK.
    const cellSettingsModal = document.getElementById('cellSettingsModal'); // Este ID não estava no seu HTML mais recente
        ...
        try {
            ...
            translations = {
                pt: {
                    ...
                    fileNameLabel: "Nome do Ficheiro:", // Estas e as seguintes são para o modal
                    ...
                    clearAllCellsButton: "Limpar Todas as Células",
                    confirmClearAllCells: "Tem certeza que deseja limpar TODAS as células? Esta ação não pode ser desfeita.",
                    toggleHelpButtonHide: "Esconder Ajuda" // Para quando a ajuda está visível
                },
                en: {
                    title: "Soundboard QWERTY",
                    mainTitle: "Soundboard QWERTY",
                    ...
                    confirmClearAllCells: "Are you sure you want to clear ALL cells? This action cannot be undone.",
                ...
                it: {
                    ...
                    clearAllCellsButton: "Cancella Tutte le Celle",
                    confirmClearAllCells: "Sei sicuro di voler cancellare TUTTE le celle? Questa azione non può essere annullata.",
                    toggleHelpButtonHide: "Nascondi Aiuto"
                }
            };
        }
    }
    ...
    function setLanguage(lang) {
        ...
        // Atualiza o texto do botão de ajuda baseado no estado atual
    ...
    async function loadSettings() {
        ...
        if (savedSettings) {
            ...
            for (let i = 0; i < NUM_CELLS; i++) {
                ...
                if (soundData[i] && soundData[i].fileName) { // Verifica se existe nome de ficheiro
                    const soundInfo = soundData[i];
                    // Tenta carregar o audioBuffer. Se falhar, a célula ficará vazia.
                    const loaded = await loadAudioBuffer(soundInfo.fileName, i);
                    if (loaded) {
                        updateCellDisplay(cell, soundInfo, false);
                        if (soundInfo.isCued) {
                            cuedSounds.add(i);
                            cell.classList.add('cued');
                        }
                    } else {
                        // Se o carregamento falhar, reseta a célula
                        soundData[i] = null;
    ...
    async function loadAudioBuffer(url, index) {
        initAudioContext();
        try {
            // Para ficheiros carregados via drag/drop ou input, o 'url' será o nome original do ficheiro.
            // Para recarregar após um refresh, precisamos do objeto File ou uma forma de recriar o URL.
            // A forma mais robusta é salvar o ArrayBuffer ou uma referência ao File.
            // Como estamos a guardar 'fileName' e 'file', vamos usar o 'file' para recarregar se disponível.
            const file = soundData[index] ? soundData[index].file : null;

            let audioBuffer;
            if (file) {
                // Se o objeto File original está guardado, carregue dele.
                const reader = new FileReader();
                reader.readAsArrayBuffer(file);
                audioBuffer = await new Promise((resolve, reject) => {
                    reader.onload = () => audioContext.decodeAudioData(reader.result).then(resolve).catch(reject);
                    reader.onerror = reject;
                });
            } else {
                // Caso contrário (e.g., para URLs hardcoded ou se 'file' não foi persistido), tente via fetch.
                // Note: fetch(url) só funcionará para URLs acessíveis publicamente ou no mesmo domínio.
                // Para ficheiros locais guardados, isto falhará.
            ...
            console.error(`Erro ao carregar ou decodificar áudio para a célula ${index} (${url}):`, error);
            // Marcar a célula como vazia em caso de erro no carregamento
            if (soundData[index]) { // Limpa apenas se já havia dados para evitar sobrescrever célula vazia
        ...
        reader.onload = async () => {
            try {
                ...
                if (!updateOnly) { // Se for um carregamento inicial ou substituição completa
                    // Para garantir que `file` é armazenado, precisamos de um campo para ele.
                    soundData[index] = {
                        fileName: file.name,
                        file: file, // Armazena o objeto File completo
                        name: soundName,
                        key: key,
                        isLooping: false,
                        audioBuffer: audioBuffer,
                    ...
                    soundData[index].file = file; // Atualiza o objeto File
    ...
    function openCellSettingsModal(index) {
        ...
        if (!sound || !cellSettingsModal) { // Verifica se o modal existe
            console.error("No sound data for index or modal not found:", index);
            return;
        }

        if (modalTitle) modalTitle.textContent = `${translations[currentLanguage].soundNameLabel} - ${sound.name}`;
        ...
        if (document.getElementById('fileNameDisplayLabel')) document.getElementById('fileNameDisplayLabel').textContent = translations[currentLanguage].fileNameLabel;
        if (document.getElementById('soundNameLabel')) document.getElementById('soundNameLabel').textContent = translations[currentLanguage].soundNameLabel;
        if (document.getElementById('keyBindingLabel')) document.getElementById('keyBindingLabel').textContent = translations[currentLanguage].keyBindingLabel;
        if (document.getElementById('loopLabel')) document.getElementById('loopLabel').textContent = translations[currentLanguage].loopLabel;
        if (document.getElementById('clearCellButton')) document.getElementById('clearCellButton').textContent = translations[currentLanguage].clearCellButton;
        if (document.getElementById('saveCellSettingsButton')) document.getElementById('saveCellSettingsButton').textContent = translations[currentLanguage].saveCellSettingsButton;
        ...
        cellSettingsModal.style.display = 'block';
    }
    ...
    function saveCellSettings(index) {
        const sound = soundData[index];
        if (sound && soundNameInput && keyBindingInput && loopCheckbox) { // Verifica se os elementos existem
    ...
    function playActualSound(sound, index, fadeInDuration = 0) {
        ...
        if (cell) {
            ...
            source.onended = () => {
                // A remoção da classe 'active' e limpeza da instância deve ocorrer
                // SOMENTE se o som não estiver mais a tocar (e.g., não está em loop e terminou)
                // ou se a instância foi explicitamente parada.
                // Um pequeno delay ajuda a garantir que o som realmente parou antes de atualizar o UI.
                if (!source.loop) {
                    setTimeout(() => {
                        // Verifica se a instância ainda existe antes de tentar apagar
                        ...
                        // Só remove a classe 'active' se não houver outras instâncias ativas
    ...
    function fadeoutInstance(sourceNode, gainNode, duration) {
        ...
        if (duration === 0) {
            ...
            gainNode.gain.setValueAtTime(0, now);
            try {
                sourceNode.stop(now); // Garante que para imediatamente
            } catch (e) {
            ...
            // Interrompe qualquer agendamento de ganho anterior para evitar conflitos
            gainNode.gain.cancelScheduledValues(now);
            gainNode.gain.setValueAtTime(gainNode.gain.value, now); // Começa do valor atual
            gainNode.gain.linearRampToValueAtTime(0.001, now + duration); // Rampa para quase zero

            const stopTime = now + duration;
            try {
                sourceNode.stop(stopTime); // Agenda a paragem para o final do fade
            } catch (e) {
                console.warn("Erro ao agendar stop para sourceNode:", e);
            }

            sourceNode.onended = () => {
                // onended é chamado quando o sourceNode termina de tocar (naturalmente ou por stop())
                // Isso garante a desconexão apenas quando o áudio realmente parou.
                sourceNode.disconnect();
                gainNode.disconnect();
            };
        }
    ...
    function fadeoutSound(index, duration) {
        ...
        // Cria uma cópia do Set para iterar, pois o Set pode ser modificado dentro do loop
        ...
        // Só remove a classe 'active' se não houver mais instâncias ativas para este som
        // Isso é importante para loops que podem estar a ser parados
        setTimeout(() => {
            if (sound.activePlayingInstances.size === 0 && cell) {
                cell.classList.remove('active');
            }
        }, duration * 1000 + 100); // Dá um pequeno tempo extra para o fade concluir
    }
    ...
    function clearSoundCell(index, fadeDuration = 0.1) {
        ...
        // Garante que o som para antes de limpar os dados
        fadeoutSound(index, fadeDuration);

        // Limpa os dados e atualiza o display após o fade (ou quase imediatamente se fadeDuration for 0)
        setTimeout(() => {
            clearSoundData(index); // Limpa os dados do array soundData
            const cell = document.querySelector(`.sound-cell[data-index="${index}"]`);
            if (cell) {
                // Redefine a célula para o estado "vazio"
                updateCellDisplay(cell, { name: translations[currentLanguage].cellEmptyDefault, key: defaultKeys[index] || '', isLooping: false, isCued: false }, true);
                cell.classList.remove('active'); // Garante que a classe 'active' é removida
                cell.classList.remove('cued'); // Garante que a classe 'cued' é removida
            }
        ...
        }, fadeDuration * 1000 + 50); // Espera o fade mais um pequeno buffer
    }

    function clearSoundData(index) {
        const sound = soundData[index];
        if (sound && sound.activePlayingInstances) {
            // Garante que todas as instâncias são removidas dos sets globais e locais
            sound.activePlayingInstances.forEach(instance => {
                globalActivePlayingInstances.delete(instance);
                try {
                    // Tenta parar e desconectar, caso ainda estejam ativos
                    instance.source.stop(0);
                    instance.source.disconnect();
                    instance.gain.disconnect();
                } catch (e) {
                    // Pode ocorrer erro se o nó já foi parado/desconectado
    ...
    function playCuedSounds() {
        ...
        const soundsToPlay = Array.from(cuedSounds).sort((a, b) => a - b); // Tocar na ordem das células
    ...
    function findNextSoundIndex(startIndex, direction) {
        ...
        let attempts = 0;
        const maxAttempts = NUM_CELLS * 2; // Duas voltas para garantir que todos os sons são verificados
        ...
        while (attempts < maxAttempts) {
            ...
            if (currentIndex >= NUM_CELLS) {
                ...
                currentIndex = NUM_CELLS - 1; // Volta ao fim
            }

            // Se o som existe e tem um buffer de áudio
            ...
            if (currentIndex === startIndex && attempts > 1) {
                return null;
            }
        }
