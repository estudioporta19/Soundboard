...
window.soundboardApp.i18n = (function() {
    ...
    function setLanguage(lang, domElements, soundData, updateCellDisplay, utils) {
        ...
        domElements.soundCells.forEach(cell => {
            ...
            if (deleteButton) {
                deleteButton.title = (translations[currentLanguage].deleteSoundHelp || 'Apagar Som').replace(/<[^>]*>/g, '');
            }
            const replaceButton = cell.querySelector('.replace-sound-button');
            if (replaceButton) {
                replaceButton.title = (translations[currentLanguage].replaceSoundHelp || 'Substituir Som').replace(/<[^>]*>/g, '');
            }
